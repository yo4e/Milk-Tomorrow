...
### Milk Tomorrow decisions

- The user selected the second generated direction, “Home Forecast,” and explicitly liked its cute tone.
- Treat `design/home-forecast-reference.png` as the visual source of truth for the default open-request state.
- Preserve the calm household-weather framing: pale sky blue, deep navy, orange forecast risk, green action, large mobile tap targets, and family-friendly copy.
- Keep the surface simple for non-technical parents and children, while the expandable Tomorrow Lab exposes the probabilistic simulation for judges.
- Treat the normal product URL as a frameless, mobile-first responsive web app: full-width on phones and a centered readable column on desktop. Preserve the calibrated phone simulator as the optional `?preview=phone` QA/demo mode rather than showing device chrome by default.
- Keep the current judged slice focused on milk. The broader pantry architecture may later add cereal and other consumables, but do not dilute this one-item demo unless the user explicitly expands the scope.
- Take the time needed for careful implementation and verification; speed is not a user constraint.
